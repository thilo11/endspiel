#!/usr/bin/env -S uv run --script
# /// script
# requires-python = ">=3.14"
# dependencies = [
#   "zstandard>=0.25.0",
# ]
# ///
"""Extract endgame FENs from `assets/lichess_db_eval.jsonl.zst` for datagen seeding.

Reads JSONL (one {fen, evals: [...]} per line), filters by piece count, and
writes one FEN per line to the output file. Scores are dropped entirely — this
is only a source of starting positions for engine self-play datagen.

Typical use:
    scripts/extract_endgame_fens.py \\
        --min-pieces 6 --max-pieces 12 \\
        --limit 200000 --dedupe \\
        --output assets/endgame_fens.txt
"""

from __future__ import annotations

import argparse
import io
import random
import sys
import time

import zstandard as zstd


def count_pieces_fast(fen: str) -> int:
    board_part = fen.split(" ", 1)[0]
    return sum(1 for c in board_part if c.isalpha())


def main() -> int:
    ap = argparse.ArgumentParser()
    ap.add_argument("--input", default="assets/lichess_db_eval.jsonl.zst")
    ap.add_argument("--output", required=True)
    ap.add_argument("--min-pieces", type=int, default=6)
    ap.add_argument("--max-pieces", type=int, default=12)
    ap.add_argument("--limit", type=int, default=None,
                    help="Stop after N kept FENs")
    ap.add_argument("--dedupe", action="store_true",
                    help="Skip duplicate FENs (board+stm only)")
    ap.add_argument("--sample-rate", type=float, default=1.0,
                    help="Keep each eligible FEN with this probability")
    ap.add_argument("--seed", type=int, default=42)
    ap.add_argument("--progress-every", type=int, default=1_000_000)
    args = ap.parse_args()

    rng = random.Random(args.seed)
    seen: set[str] = set()
    kept = 0
    lines = 0
    skipped_pieces = 0
    skipped_dup = 0
    skipped_sample = 0
    t0 = time.time()

    out = open(args.output, "w", encoding="utf-8")
    try:
        with open(args.input, "rb") as raw:
            dctx = zstd.ZstdDecompressor()
            with dctx.stream_reader(raw) as zr:
                text = io.TextIOWrapper(zr, encoding="utf-8", newline="")
                for line in text:
                    lines += 1
                    # Locate the FEN without json.loads — much faster.
                    i = line.find('"fen":"')
                    if i < 0:
                        continue
                    j = line.find('"', i + 7)
                    if j < 0:
                        continue
                    fen = line[i + 7:j]

                    pieces = count_pieces_fast(fen)
                    if pieces < args.min_pieces or pieces > args.max_pieces:
                        skipped_pieces += 1
                        continue

                    if args.sample_rate < 1.0 and rng.random() >= args.sample_rate:
                        skipped_sample += 1
                        continue

                    # lichess FENs are 4-field; pad to 6 so chess-datagen's
                    # from_fen accepts them directly.
                    parts = fen.split()
                    if len(parts) == 4:
                        fen_full = fen + " 0 1"
                    else:
                        fen_full = fen

                    if args.dedupe:
                        # Key by first two fields (board + stm); castling/ep
                        # are zero in 4-field FENs, so this is safe here.
                        key = " ".join(fen_full.split()[:2])
                        if key in seen:
                            skipped_dup += 1
                            continue
                        seen.add(key)

                    out.write(fen_full)
                    out.write("\n")
                    kept += 1

                    if args.limit and kept >= args.limit:
                        break

                    if lines % args.progress_every == 0:
                        elapsed = time.time() - t0
                        rate = lines / elapsed if elapsed > 0 else 0
                        print(
                            f"  lines={lines:,} kept={kept:,} "
                            f"rate={rate/1e3:.1f}k/s elapsed={elapsed/60:.1f}min",
                            file=sys.stderr, flush=True,
                        )
    finally:
        out.close()

    elapsed = time.time() - t0
    print(file=sys.stderr)
    print("=== STATS ===", file=sys.stderr)
    print(f"  lines read       : {lines:,}", file=sys.stderr)
    print(f"  skipped (pieces) : {skipped_pieces:,}", file=sys.stderr)
    print(f"  skipped (dup)    : {skipped_dup:,}", file=sys.stderr)
    print(f"  skipped (sample) : {skipped_sample:,}", file=sys.stderr)
    print(f"  kept             : {kept:,}", file=sys.stderr)
    print(f"  elapsed          : {elapsed/60:.1f} min", file=sys.stderr)
    print(f"  output           : {args.output}", file=sys.stderr)
    return 0


if __name__ == "__main__":
    sys.exit(main())
